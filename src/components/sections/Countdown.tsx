...
function CountdownBox({ value, label }: CountdownBoxProps) {
  return (
    <div className="rounded-2xl border border-saiyan/30 bg-paper px-1.5 py-4 pb-3 shadow-[0_14px_34px_-26px_rgba(59,55,66,.5)]">
      <div
        key={value}
...
export function Countdown() {
  ...
  return (
    <section
      className="bg-cover bg-center py-[clamp(30px,8vw,52px)] pb-[clamp(46px,11vw,66px)] text-center"
      style={{ backgroundImage: `url(${EVENT.countdownBgImage})` }}
    >
      <div className="mx-auto w-full max-w-[560px] px-[clamp(22px,6vw,34px)]">
        <RevealOnScroll className="mb-[34px]">
          ...
          <h2 className="mt-2 font-serif text-[clamp(1.9rem,8vw,2.5rem)] font-medium text-ink">Falta muy poco</h2>
        ...
        <RevealOnScroll className="relative">
          <div
            ...
            style={{ background: 'radial-gradient(70% 70% at 50% 40%, rgba(232,121,43,.22), transparent 66%)' }}
          />
          {isDone ? (
            <p className="font-emot text-[1.6rem] italic text-ink">{EVENT.countdownDoneMessage}</p>
          ) : (
        ...
        {!isDone && (
          <RevealOnScroll>
            <p className="mt-[26px] font-emot text-[1.15rem] italic text-inkSoft">{EVENT.countdownNote}</p>
          </RevealOnScroll>
        )}
      </div>
    </section>
